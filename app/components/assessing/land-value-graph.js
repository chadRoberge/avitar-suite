...
export default class LandValueGraphComponent extends Component {
  @tracked svg = null;
  ...
  drawGraph(container) {
    ...
    if (!container || !this.args.tiers?.length) return;

    const tiers = this.args.tiers || [];
    ...
    // Sort tiers by acreage
    const sortedTiers = [...tiers].sort((a, b) => a.acreage - b.acreage);

    // Create scales
    const xScale = d3
      .scaleLinear()
      .domain(d3.extent(sortedTiers, (d) => d.acreage))
      .range([0, width]);

    const yScale = d3
      .scaleLinear()
      .domain(d3.extent(sortedTiers, (d) => d.value))
      .range([height, 0]);
