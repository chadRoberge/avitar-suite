/**
 * Client-side PID formatting utility
 ...
 */
export function formatPid(pidRaw, municipalityConfig) {
 ...
 */
export function parsePidParts(pidRaw, config) {
 ...
 */
export function formatParts(parts, config) {
 ...
 */
export function getPidConfig(municipality) {
  if (!municipality) {
    return null;
  }
 ...
 */
export function getMapFromPid(pidRaw, municipality) {
  if (!pidRaw || pidRaw.length !== 18) {
    return 'Unknown';
  }
 ...
 */
export function getLotSubFromPid(pidRaw, municipality) {
  if (!pidRaw || pidRaw.length !== 18) {
    return 'Unknown';
  }
...
/**
 * Apply PID formatting to a property object
 * Adds mapNumber, lotSubDisplay, and pid_formatted fields
 * @param {object} property - Property object with pid_raw field
 * @param {object} municipality - Municipality configuration
 * @returns {object} - Property with PID formatting applied
 */
export function applyPidFormatting(property, municipality) {
  if (!property || !property.pid_raw) {
    return property;
  }
...
/**
 * Apply PID formatting to an array of properties
 * @param {array} properties - Array of property objects
 * @param {object} municipality - Municipality configuration
 * @returns {array} - Properties with PID formatting applied
 */
export function applyPidFormattingBulk(properties, municipality) {
