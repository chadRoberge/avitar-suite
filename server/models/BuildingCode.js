...
buildingCodeSchema.index(
  { code: 1, municipalityId: 1, effective_year: 1 },
  { unique: true },
);
buildingCodeSchema.index({ municipalityId: 1, effective_year: 1 });
...
buildingCodeSchema.statics.findByMunicipalityForYear = async function (
  municipalityId,
  year,
) {
...
// Find a specific code by code string for a year
// Uses temporal range: effective_year <= year AND (effective_year_end is null OR effective_year_end > year)
buildingCodeSchema.statics.findByCodeForYear = async function (
  municipalityId,
  code,
  year,
) {
...
// Get distinct years that have building code data for a municipality
buildingCodeSchema.statics.getAvailableYears = async function (municipalityId) {
  const objectId = new mongoose.Types.ObjectId(municipalityId);
