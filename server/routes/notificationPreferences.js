 ...
 */
router.get('/users/me/notification-preferences', authenticateToken, async (req, res) => {
  try {
 ...
 */
router.patch('/users/me/notification-preferences', authenticateToken, async (req, res) => {
  try {
    const user = await User.findById(req.user._id);
    ...
    if (sms_carrier !== undefined) {
      const validCarriers = ['verizon', 'att', 'tmobile', 'sprint', 'us_cellular', 'boost', 'cricket', 'metro_pcs', 'other'];
 ...
 */
router.get('/notification-preferences/carriers', authenticateToken, async (req, res) => {
  try {
    const carriers = [
      ...
      { value: 'us_cellular', label: 'U.S. Cellular', gateway: '@email.uscc.net' },
      { value: 'boost', label: 'Boost Mobile', gateway: '@smsmyboostmobile.com' },
 ...
 */
router.post('/users/me/test-notification', authenticateToken, async (req, res) => {
  try {
